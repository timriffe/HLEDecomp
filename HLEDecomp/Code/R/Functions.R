...
# deprecated?
get_rates_all <-
  function(path = "N:\\dcs\\proj\\hledecomp\\results",
           version = "01") {
    final_path    <-
      file.path(path,
                "margins",
                paste0("mspec", version),
                paste0("transp_m", version, ".dta"))
...
out2self <- function(datout, ntrans = 3) {
  ...
  out.order       <-
    as.data.frame(matrix(trans.out, ntrans, byrow = TRUE), stringsAsFactors =
                    FALSE)
...
data_2_U <- function(datself, ntrans = 3, dead = as.character(ntrans+1)) {
  ...
  this.order <-
    as.data.frame(matrix(trans.self, ntrans, byrow = FALSE), stringsAsFactors =
                    FALSE)
  
  # take advantage of data.frame columns as list elements
  UL         <-
    lapply(as.data.frame(datself[, trans.self], stringsAsFactors = FALSE), pi2u)
...
do_decomp <- function(years = c(1996, 2014),
                      ntrans,
                      # number of states
                      version = "02",
                      # character 2 digits
                      ...
                      edu = "all_edu",
                      to,
                      # i.e. do we decompose wrt HLE, ADL1, ADL2p, or LE. leave missing for LE
                      N = 20,
                      deduct = TRUE) {
  years <- years[1:2]
...
collapseTR   <- function(TR, PREV, version = "02") {
  ...
  m21        <-
    (RP$m21 * RP$pi2 + RP$m31 * RP$pi3) / (RP$pi2 + RP$pi3)
  m24        <-
    (RP$m24 * RP$pi2 + RP$m34 * RP$pi3) / (RP$pi2 + RP$pi3)
  m22        <- 1 - m21 - m24
  m22i       <-
    ((RP$m22 + RP$m23) * RP$pi2 + (RP$m33 + RP$m32) * RP$pi3) / (RP$pi2 + RP$pi3)
  ...
  RP         <-
    cbind(RP[, c("time", "sex", "edu", "age", "s1_prop")],
