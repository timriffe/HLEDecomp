...
horiuchi_constrain <- function(func, pars1, pars2, N, ...){
	...
	for (j in 1:N){
		...
		for (i in 1:n){
			...
			# this is a bit different in that we specify which element gets perturbed
			# and by how much. Perturbation is outsourced to our own functions that know
			# how to take care of rescaling. The indicator arg is called ii, sorry
			cc[i,j] <- func(x[, j], ii = i, delta = delta[i]/2, ...) - 
					   func(x[, j], ii = i, delta = -delta[i]/2, ...)
		}
	}	
...
f_dec_rescale_i <- function(vecall,ii,delta,to=1,age=52,ntrans=2,deduct=TRUE){
	...
	# TODO: this needs to add delta to the iith element, and rescale the 
	# REMAINING elements. Presently rescaling all of them.
	for (i in 1:ntrans){
		iiii<- ((i-1)*(ntrans+1) +1):(i*(ntrans+1))
...
f_dec_rescale_all_edu_dt <- function(.SD,to=1,age=50,ntrans=2,deduct=TRUE){
	...
	eduprop <- rowSums(.SD[age==50, paste0("s", 1:ntrans, "_prop")])
