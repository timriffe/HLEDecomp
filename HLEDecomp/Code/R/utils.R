...
# m11, m12, m13 etc
getcols <- function(ntrans = 3,
                    self = TRUE,
                    dead = "4") {
  if (self) {
    return(paste0("m", c(t(
      outer(1:ntrans, 1:ntrans, paste0)
    ))))
  } else {
  ...
  function(prev,
    ...
    if (type == "bar") {
      ...
      barplot(
        ...
        border = NA,
        xlab = "Age",
        ylab = ifelse(scale, "Proportion", "Prevalence"),
        col = col,
        ...
      )
...
barmargins <- function(dec.i, ylim) {
  ...
  sex           <-
    ifelse(sex == "1.men" , "m", ifelse(sex == "2.wmn", "f", "b"))
  
  edus          <-
    c("all_edu", "primary" , "secondary", "terciary")
  edus1         <-
    c("0.All edu", "1.Less HS" , "4.HS/GED/Sm coll ex AA", "5.AA/BS/+")
  names(edus)   <- edus1
  edu           <- edus[unique(educlevel)]
  
  
  trmargins     <-
    acast(dec.i, transition ~ state ~ decnr, sum, value.var = "value")
  ...
  if (missing(ylim)) {
    ylim <-
      range(pretty(c(
        apply(trn, 3, rowSums, na.rm = TRUE),
        apply(trp, 3, rowSums, na.rm = TRUE)
      )))
  }
  ...
  barplot(
    t(trp[, , 1]),
    ylim = ylim,
    legend.text = c("HLE", "ADL1", "ADL2p"),
    main = paste(code[1], sex, edu),
    ylab = "contribution to difference in e50",
    space = 0
  )
  text(
    1:length(lab.extra) - .5,
    ylim[1] - .25,
    names(lab.extra),
    srt = 90,
    xpd = TRUE,
  ...
  barplot(
    t(trp[, , 3]),
    ylim = range(trmargins),
    legend.text = c("HLE", "ADL1", "ADL2p"),
    main = paste(code[3], sex, edu),
    ylab = "contribution to difference in e50",
    space = 0
  )
  text(
